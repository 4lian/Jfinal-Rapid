...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="<#noparse>${PATH}</#noparse>/${entity.name}/add" target="navTab" rel="rel_${entity.name}_add"><span>添加</span></a></li>
			<li><a class="delete" href="<#noparse>${PATH}</#noparse>/${entity.name}/delete/{sid_user}" rel="rel_${entity.name}_delete" target="ajaxTodo" title="确定要删除吗?"><span>删除</span></a></li>
			<li><a class="edit" href="<#noparse>${PATH}</#noparse>/${entity.name}/edit/{sid_user}" target="navTab" rel="rel_${entity.name}_edit"><span>修改</span></a></li>
			<li class="line">line</li>
			<li><a class="icon" href="<#noparse>${PATH}</#noparse>/${entity.name}/download" target="dwzExport" targetType="navTab" rel="rel_${entity.name}_dowmload" title="实要导出这些记录吗?"><span>导出EXCEL</span></a></li>
		</ul>
	</div>
	<table class="table" width="100%" layoutH="138">
		...
		<tbody>
			...
			<tr target="sid_user" rel="<#noparse>${item.id}</#noparse>">
				...
				<td><#noparse>${(</#noparse>item.${field.name}<#noparse>)!}</#noparse></td>
				</#if>
				</#list>
			</tr>
