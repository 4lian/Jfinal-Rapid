/*
 * Bookshelf Slider jQuery Plugin - v1.2
 * Author: Sergio Valle
 * Date: Mar 26, 2012
 * 
...
(function($){
	...
	$.bookshelfSlider = function(selector, settings) {
		...
		$(selector + ' .product').each(function(i) {
			...
			if ($prod.attr("detailTitle") != "" && $prod.attr("detailTitle") != "undefined") {
				var has_title = true;	
			} else {
				var has_title = false;
			}
			...
			if (has_title && $bsc.product_show_title) {
				$prod.append("<div class='product_title' id='title_"+i+"'><p>"+$prod.attr('detailTitle')+"</p></div>");
				...
				the_title.css({ color: $bsc.product_title_textcolor, background: $bsc.product_title_bgcolor + ' url(../images/assets/title_product_bg.png) repeat' });
			}
			...
			if (data_type == 'book') {
				$prod.append("<img class='fx_book' id='b_" + i + "' + src='../images/assets/book_fx.png' />");
			...
			//add shadow
			$prod.append("<img class='fx_shadow' id='shd_" + i + "' + src='../images/assets/shadow_fx.png' />");
		...
		$(selector + ' .product').click(function(e) {
			...
			$('#popup_info').append("<img id='preload_icon' src='../../images/assets/preload.gif' />");
...
//					}
		...
		$(window).resize(function() {
			...
			//panels resize
			$(selector).width( $(window).width() - 10);
